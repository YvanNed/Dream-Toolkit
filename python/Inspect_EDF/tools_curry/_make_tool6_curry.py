...
# ===========================================================================
print("\n=== Curry-only: per-channel rejection (memory) + progress feedback ===")

# --- Cell 2: replace compute_rejection_masks with the per-channel variant -----------------
# Reads the signal one channel at a time from `epochs` instead of the full epochs_data_uV
# array, so a high-density montage is never held as a 2nd (n_epochs, n_ch, n_times) float64
# array. Formulas identical -> per-(epoch, channel) masks byte-identical. Adds a `progress`
# callback (per channel for the time-domain pass, then per epoch for the 1/f fits).
NEW_REJECT = '''def compute_rejection_masks(epochs, sfreq, hypno_epochs, ptp_thresholds,
...
_s2 = get_src(shared)
_a = _s2.find("def compute_rejection_masks(")
_b = _s2.find("\n\n\ndef build_combined_method_matrix")
if _a == -1 or _b == -1:
    errors.append("compute_rejection_masks anchors"); print("  X compute_rejection_masks anchors")
else:
    set_src(shared, _s2[:_a] + NEW_REJECT + _s2[_b:])
    print("  ok cell2 per-channel compute_rejection_masks")

# --- Cell 8: progress feedback ------------------------------------------------------------
# The EDF original now owns the per-participant pipeline bar (progress_step + step_legend +
# 4-segment legend), inherited here unchanged. For Curry we no longer add a separate rejection
# sub-bar: instead the per-channel/per-epoch callback drives the pipeline bar's Reject segment
# (see the reject call site below), so EDF and Curry share the exact same two-bar layout.
# The phase text labels come from set_phase() in the EDF original (inherited unchanged).

# free raw right after epoching (raw not used afterwards)
replace_in_cell(run_cell,
...
# drop the full epochs_data_uV array
replace_in_cell(run_cell,
    "            hypno_epochs   = expert_hypno[:n_epochs]\n"
    "            epochs_data_uV = epochs.get_data() * 1e6  # (n_epochs, n_channels, n_times)",
    "            hypno_epochs   = expert_hypno[:n_epochs]",
    "cell8 drop epochs_data_uV")

# rejection: pass `epochs` (not the full array) + a per-channel/per-epoch callback that drives
# the pipeline bar's Reject segment (progress_step, within [_reject_base, _reject_base+COST_REJECT]),
# so Curry animates smoothly through the slow high-density rejection while sharing the EDF layout.
# The set_phase('rejecting epochs…') text label is already in the EDF original.
replace_in_cell(run_cell,
    ...
    "                    fit_fmin=fit_fmin_val, fit_fmax=fit_fmax_val\n"
    "                )",
    "            # Masques de rejet — fatal\n"
    "            set_phase('rejecting epochs…')\n"
    "            try:\n"
